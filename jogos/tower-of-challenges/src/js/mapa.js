...
export default class mapa extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.spritesheet("cavaleiro-1", "./assets/entities/kingone.png", {
      frameWidth: 32,
      frameHeight: 32,
    });
    this.load.spritesheet("cavaleiro-2", "./assets/entities/kingtwo.png", {
      frameWidth: 32,
      frameHeight: 32,
    });
    this.load.spritesheet("corvo-idle", "./assets/entities/raven_idle.png", {
      frameWidth: 32,
      frameHeight: 32,
    });
    this.load.spritesheet("corvo-fly", "./assets/entities/raven_fly.png", {
      frameWidth: 48,
      frameHeight: 32,
    });
    this.load.spritesheet("rainall", "./assets/particles/rainall.png", {
      frameWidth: 120,
      frameHeight: 120,
    });
    ...
    this.load.spritesheet("jump", "./assets/ui/jump.png", {
      frameWidth: 64,
      frameHeight: 64,
    });
    this.load.spritesheet("baixo", "./assets/ui/baixo.png", {
      frameWidth: 64,
      frameHeight: 64,
    });
    this.load.spritesheet("direita", "./assets/ui/direita.png", {
      frameWidth: 64,
      frameHeight: 64,
    });
    this.load.spritesheet("esquerda", "./assets/ui/esquerda.png", {
      frameWidth: 64,
      frameHeight: 64,
    });
    this.load.spritesheet("progress", "./assets/ui/progress.png", {
      frameWidth: 16,
      frameHeight: 32,
    });
    this.load.spritesheet("fogueira", "./assets/decoracao/bonfire.png", {
      frameWidth: 32,
      frameHeight: 32,
    });
    this.load.spritesheet("torch", "./assets/decoracao/torch.png", {
      frameWidth: 32,
      frameHeight: 32,
    });
    this.load.spritesheet("coroa", "./assets/decoracao/coroa.png", {
      frameWidth: 32,
      frameHeight: 32,
    });
    this.load.spritesheet("skeleton", "./assets/decoracao/skeleton.png", {
      frameWidth: 24,
      frameHeight: 24,
    });
  }
  ...
  create() {
    ...
    this.layerTerreno = this.tilemapMapa.createLayer("terreno", [
      this.tilesetBlocos,
    ]);
    this.layerBlocos = this.tilemapMapa.createLayer("blocos", [
      this.tilesetBlocos,
    ]);
    this.layerDecoration = this.tilemapMapa.createLayer("decoracao-1", [
      this.tilesetBlocos,
    ]);
    this.layerDecoration2 = this.tilemapMapa.createLayer("decoracao-2", [
      this.tilesetBlocos,
    ]);
    ...
    this.anims.create({
      key: "corvo-idle",
      frames: this.anims.generateFrameNumbers("corvo-idle", {
        start: 0,
        end: 3,
      }),
      frameRate: 2,
    ...
    this.anims.create({
      key: "corvo-fly",
      frames: this.anims.generateFrameNumbers("corvo-fly", {
        start: 0,
        end: 2,
      }),
      frameRate: 5,
    ...
    this.anims.create({
      key: "progress",
      frames: this.anims.generateFrameNumbers("progress", {
        start: 0,
        end: 32,
      }),
      frameRate: 23,
    ...
    this.anims.create({
      key: "particula_jump",
      frames: this.anims.generateFrameNumbers("particula_jump", {
        start: 0,
        end: 4,
      }),
      frameRate: 20,
    ...
    this.anims.create({
      ...
      frames: this.anims.generateFrameNumbers(this.personagem.texture.key, {
        start: 22,
        end: 24,
      }),
      frameRate: 5,
      ...
      .on("pointerdown", () => {
        if (this.personagem.body.blocked.down) {
          ...
          this.progress = this.add.sprite(
            this.personagem.body.x + 40,
            this.personagem.body.y + 15,
            "progress",
          );
      ...
      .on("pointerup", () => {
        if (this.personagem.body.blocked.down) {
          ...
          if (jumping && jumpTimer <= maxJumpTime) {
            jumping = false;
            if (this.particula_jump) {
              this.particula_jump.destroy();
            }
            this.particula_jump = this.add.sprite(
              this.personagem.body.x,
              this.personagem.body.y + 15,
              "particula_jump",
            );
            this.particula_jump.anims.play("particula_jump");
            var jumpFactor = jumpTimer / maxJumpTime;
            var currentJumpForceX =
              (jumpForce - jumpForce * jumpFactor) * dir_lados;
  ...
  update() {
    ...
    if (!this.personagem.body.blocked.down) {
      if (
        this.personagem.body.velocity.y > 0 &&
        this.dir_direita &&
        !this.bounced
      ) {
    ...
    if (this.personagem.body.blocked.down && this.entrar && this.dir_esquerda) {
      ...
      this.personagem.body.blocked.down &&
      this.entrar &&
      this.dir_direita
    ) {
      console.log("teste");
