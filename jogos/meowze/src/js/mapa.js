...
export default class mapa extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.spritesheet("Boo", "./assets/personagens/Boo.png", {
      frameWidth: 32,
      frameHeight: 32,
    });
    this.load.spritesheet("Missy", "./assets/personagens/Missy.png", {
      frameWidth: 32,
      frameHeight: 32,
    });

    // Carrega o Vilão SUPREMO
    this.load.spritesheet("cachorro", "./assets/personagens/cachorro.png", {
      frameWidth: 32,
      frameHeight: 32,
    });
    ...
    this.load.spritesheet("cookie", "./assets/cookies.png", {
      frameWidth: 32,
      frameHeight: 32,
    });
    this.load.spritesheet("moedinha", "./assets/moedinha.png", {
      frameWidth: 32,
      frameHeight: 32,
    });
  }
  ...
  create() {
    ...
    this.layerGrama = this.tilemapMapa.createLayer("grama", [
      this.tilesetGrama,
    ]);
    this.layerCerca = this.tilemapMapa.createLayer("cerca", [
      this.tilesetCerca,
    ]);
    this.layerCasa = this.tilemapMapa.createLayer("casa", [this.tilesetCasa]);
    this.layerArbusto = this.tilemapMapa.createLayer("arbusto", [
      this.tilesetArvore,
    ]);
    ...
    this.moedinha.forEach((moedinha) => {
      moedinha.objeto = this.physics.add.sprite(
        moedinha.x,
        moedinha.y,
        "moedinha",
      );
      moedinha.objeto.anims.play("moedinha");
      moedinha.colisao = this.physics.add.overlap(
        this.personagemLocal,
        moedinha.objeto,
        () => {
          this.moeda.play();
    ...
    this.cookies.forEach((cookie) => {
      ...
      cookie.colisao = this.physics.add.overlap(
        this.personagemLocal,
        cookie.objeto,
        () => {
          this.comendo.play();
  ...
  update() {
    ...
    try {
      ...
      if (globalThis.game.dadosJogo.readyState === "open") {
        ...
        if (this.moedinha) {
          ...
          globalThis.game.dadosJogo.send(
            JSON.stringify({
              moedinha: this.moedinha.map((moedinha) =>
                ((moedinha) => ({
    ...
    const moedinhaColetada = this.moedinha.filter(
      (moedinha) => !moedinha.objeto.active,
    ).length;
    ...
    const cookiesColetados = this.cookies.filter(
      (cookie) => !cookie.objeto.active,
    ).length;
