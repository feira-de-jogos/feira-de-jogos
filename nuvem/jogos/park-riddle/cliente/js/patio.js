...
export default class patio extends Phaser.Scene {
  ...
  create() {
    ...
    this.layerCaminho = this.tilemapMapa.createLayer("caminho", [
      this.tilesetChao,
    ]);
    ...
    this.areaColeta = this.physics.add
      .image(0, 0, null)
      .setSize(32, 40)
      .setVisible(false);

    // Contador de pistas
    let contador = this.add
      .text(38, 45, "Pistas: 0 / 6", {
        fontFamily: "Arial",
        fontSize: 16,
        color: "#ffff00",
      })
    ...
    this.layerPlayerSobrepoe = this.tilemapMapa.createLayer("playerSobrepoe", [
      this.tilesetTendas,
      this.tilesetArvores,
    ]);
    ...
    if (this.game.jogadores.primeiro === this.game.socket.id) {
      ...
      this.barraStamina = this.add.circle(
        0,
        0,
        this.personagemLocal.stamina / 50,
        0x1a620e
      );
      ...
      //Botão de corrida
      this.botaoCorrida = this.add
        .sprite(700, 400, "corrida", 0)
        .setInteractive()
        .setScrollFactor(0);
      this.botaoCorrida
        .on("pointerdown", () => {
      ...
      //Botão de Ação
      this.botaoAcao = this.add
        .sprite(750, 350, "pista", 0)
        .setInteractive()
        .setScrollFactor(0);
      this.botaoAcao.depth = 200;

      //Animação do Cachimbo
      this.particulaAcaoLocal = this.physics.add
        .sprite(0, 0, "fumaca", 0)
        .setVisible(false)
        .setActive(false);
      ...
      this.particulaAcaoRemota = this.add
        .image(this.personagemRemoto.x, this.personagemRemoto.y, "bullet")
        .setDisplaySize(5, 5)
        .setTint(0xffff00)
        .setBlendMode(Phaser.BlendModes.ADD);
      ...
      this.botaoAcao.on("pointerdown", () => {
        ...
        this.pistas.forEach((pista) => {
          if (pista.objeto.visible) {
            sobra_pista = 1;
            distancia_pista = Phaser.Math.Distance.Between(
            ...
            if (distancia_pista < mais_proxima) {
              ...
              this.maisProximaAngulo = Phaser.Math.Angle.Between(
                this.personagemLocal.x,
                this.personagemLocal.y,
                pista.x,
                pista.y
        ...
        if (!this.particulaAcaoLocal.visible) {
          ...
          this.personagemLocal.anims.play(
            `personagem-acao-${this.direcaoAtual}`,
            true
          );
          ...
          if (this.particulaAcaoLocal.movendo == false) {
            ...
            this.time.delayedCall(1000, () => {
              ...
              this.particulaAcaoLocal
                .setPosition(
                  this.personagemLocal.x + Math.cos(this.ultimoAngulo) * 30,
                  this.personagemLocal.y + Math.sin(this.ultimoAngulo) * 30
                )
                .setFrame(0)
                .setVisible(true)
                .setActive(true);
              this.time.delayedCall(150, () => {
                ...
                this.particulaAcaoLocal.setVelocity(
                  Math.cos(this.maisProximaAngulo) * 35 * sobra_pista,
                  Math.sin(this.maisProximaAngulo) * 35 * sobra_pista
                );
              });
              this.time.delayedCall(1000, () => {
          ...
          if (this.particulaAcaoLocal.movendo == false) {
            ...
            this.time.delayedCall(700, () => {
              ...
              this.particulaAcaoLocal
                ...
                .setActive(true)
                .setVelocity(
                  Math.cos(this.ultimoAngulo) * 1000,
                  Math.sin(this.ultimoAngulo) * 1000
                );

              this.time.delayedCall(100, () => {
                const frame_tiro = this.anims
                  .get(`personagem-acao-${this.direcaoAtual}`)
                  .getFrameAt(8);
      ...
      this.fantasma = this.physics.add
        .sprite(100, -100, "fantasma")
        .setVisible(false);
      ...
      this.fantasma.on("comecou", () => {
        this.time.delayedCall(30000, () => {
          //Primeiro fantasma >>>>> 30s
          this.fantasma.setPosition(
            this.personagemRemoto.x,
            this.personagemRemoto.y - 100
          );
      ...
      this.fantasma.on("invisivel", () => {
        ...
        this.time.delayedCall(15000, () => {
          //tempo de spawn dos fantasmas >>>> 15s
          this.fantasma.setPosition(
            this.personagemRemoto.x,
            this.personagemRemoto.y - 100
          );
    ...
    this.game.dadosJogo.onmessage = (event) => {
      ...
      if (dados.alvo) {
        if (
          dados.alvo.correto.x != this.fantAlvoCorreto.x ||
          dados.alvo.correto.y != this.fantAlvoCorreto.y
        ) {
    ...
    this.layerTendas = this.tilemapMapa.createLayer("tendas", [
      this.tilesetTendas,
    ]);
    ...
    this.layerTendas.forEachTile((tile) => {
      if (tile.properties && tile.properties.lado === true) {
        ...
        this.colisaoTenda = [
          this.physics.add
            .staticImage(worldX, worldY)
            .setSize(tamanho * 2, tamanho * 2)
            .setCircle(tamanho),
          this.physics.add
            .staticImage(worldX, worldY + 64)
            .setSize(tamanho, tamanho)
            .setCircle(tamanho / 2),
          this.physics.add
            .staticImage(worldX, worldY - 48)
            .setSize(tamanho + 16, tamanho + 16)
            .setCircle((tamanho + 16) / 2),
        ];
    ...
    //Fisica do player
    this.cameras.main
      .startFollow(this.personagemLocal, true, 0.05, 0.05)
      .setBounds(0, 0, 1920, 1280);

    this.physics.world.setBounds(0, -16, 1920, 1260);
    ...
    this.physics.add.collider(this.personagemLocal, this.portao, () => {
      if (this.chaveAzul.pega && this.chaveVermelha.pega) {
        ...
        if (this.personagemLocal.texture.key == "ernesto")
          this.personagemLocal.setPosition(936, 96);
        if (this.personagemLocal.texture.key == "dan")
          this.personagemLocal.setPosition(1000, 96);
      } else {
    ...
    this.physics.add.collider(this.personagemLocal, [
      this.layerObjetos,
      this.layerTendas,
      this.layerPlayerSobrepoe,
    ]);

    this.imagemMascaraPersonagem = this.add
      .image(0, 0, "mascaraPersonagem")
      .setVisible(false);
    this.imagemMascaraLanterna = this.add
      .image(0, 0, "mascaraLanterna")
      .setVisible(false);
    ...
    // //chuva
    this.particulaChuva = this.add
      .particles(0, 0, "particula-chuva", {
        x: {
          min: this.personagemLocal.x - 1500,
          max: this.personagemLocal.x + 500,
        },
    ...
    this.ponteiro = this.physics.add
      .image(this.personagemRemoto.x, this.personagemRemoto.y, "ponteiro")
    ...
    this.add
      .image(355, 1750, "mascaraPersonagem")
    ...
    this.bordasProtecao = [
      this.physics.add
        .staticImage(150, 1750, null)
        .setVisible(false)
        .setSize(16, 400),
      this.physics.add
        .staticImage(355, 1950, null)
        .setVisible(false)
        .setSize(400, 16),
      this.physics.add
        .staticImage(355, 1550, null)
        .setVisible(false)
        .setSize(400, 16),
      this.physics.add
        .staticImage(550, 1750, null)
        .setVisible(false)
        .setSize(16, 400),
    ];
    ...
    this.physics.add.overlap(this.entradaTendaE, this.personagemLocal, () => {
      ...
      if (!flagFade) {
        ...
        this.cameras.main.once("camerafadeoutcomplete", () => {
          ...
          this.physics.world.setBounds(0, 1450, 1920, 1280);
          this.cameras.main.setBounds(-50, 1450, 260, 680);
    ...
    if (this.personagemLocal.texture.key == "dan") {
      ...
      this.bancoPosicao.forEach((banco) => {
        ...
        banco.container.detector = this.physics.add
          .image(0, 0, null)
          .setVisible(false)
          .setSize(22, 28);
        ...
        banco.spot = this.physics.add
          .image(banco.x, banco.y + 200, "marcacao")
          .setMask(this.mascaraLanterna);
        banco.spot.numero = banco.cod;
        this.spots.push(banco.spot);

        this.physics.add.overlap(
          banco.container.detector,
      ...
      this.spots.forEach((spot) => {
        ...
        this.bancosContainers.forEach((bancos) => {
          ...
          this.physics.add.overlap(spot, bancos, () => {
            ...
            if (
              !this.physics.overlap(bancos.detector, this.personagemLocal) &&
              !spot.cheio
            ) {
              spot.cheio = true;
              bancos.x = spot.x;
      ...
      this.fadeOutTween = this.tweens.add({
        ...
        onComplete: () => {
          ...
          if (this.pontosTiroAlvo < 10) {
            ...
            this.minigameAlvo.emit("ganhouAlvo");
          }
        },
      });
    ...
    this.papelEnigma1 = this.physics.add
      .image(350, 1750, "enigma1")
      .setDisplaySize(32, 40);
    this.botaoLer = this.add
      .image(this.papelEnigma1.x, this.papelEnigma1.y, "ler")
      .setInteractive();
    ...
    this.physics.add.overlap(this.papelEnigma1, this.areaColeta, () => {
      this.time.delayedCall(100, () => {
        this.botaoLer.setVisible(
          this.physics.overlap(this.papelEnigma1, this.areaColeta)
        );
      });
    });
    ...
    this.flagBlur = false;
    this.entradaTendaD = this.physics.add
      .staticImage(1630, 370)
      .setSize(50, 50);
    ...
    this.add
      .image(1560, 1750, "mascaraPersonagem")
      .setDisplaySize(800, 800)
      .setTint(0xfffabe)
      .setAlpha(0.2)
      .setBlendMode(Phaser.BlendModes.ADD);
    ...
    const povAlvos = this.add
      .image(1580, 1860, "povTiroAlvo")
      .setDisplaySize(800, 500);
    povAlvos.setVisible(false).depth = 300;

    const povAlvosSair = this.add
      .text(1580, 2060, "Clique aqui para sair")
      .setInteractive()
      .setOrigin(0.5, 0.5);
    povAlvosSair.setActive(false).setVisible(false).depth = 300;

    this.povAlvosContador = this.add.text(
      1380,
      2060,
      `Pontos: ${this.pontosTiroAlvo}`,
      {
        fontFamily: "Arial",
        fontSize: 24,
        color: "#fff000",
    ...
    this.povAlvosContador.setOrigin(0.5, 0.5).setDepth(300).setVisible(false);

    this.botaoTiro = this.add
      .image(areaTiro.x, areaTiro.y, "ler")
      .setInteractive();
    this.botaoTiro.setVisible(false);

    this.physics.add.overlap(areaTiro, this.areaColeta, () => {
      this.time.delayedCall(100, () => {
        this.botaoTiro.setVisible(
          this.physics.overlap(areaTiro, this.areaColeta)
        );
      });
    });
    ...
    this.physics.add.overlap(this.entradaTendaD, this.personagemLocal, () => {
      ...
      if (!flagFade) {
        ...
        this.cameras.main.once("camerafadeoutcomplete", () => {
          ...
          if (
            this.personagemLocal.texture.key == "ernesto" &&
            !this.flagBlur &&
            this.entrouTendaD
          ) {
          ...
          this.physics.world.setBounds(0, 1450, 1920, 1280);
          this.cameras.main.setBounds(1180, 1450, 260, 680);
  ...
  TextoFala(fala, falante, chainFunc) {
    ...
    var bkg = this.add
      .image(0, 450, `${falante}Fala`)
      .setOrigin(0, 1)
      .setScrollFactor(0)
      .setDisplaySize(800, 128)
      .setDepth(300);
    var bubble = this.add
      .graphics({ x: 140, y: 336 })
      .setScrollFactor(0)
      .setDepth(301);
    ...
    var content = this.add.text(0, 0, fala, {
      fontFamily: "Arial",
      fontSize: 20,
      color: "#bbbbbb",
      align: "center",
  ...
  EntrandoParque() {
    ...
    this.TextoFala(
      "Finalmente chegamos, bem isolado esse lugar, essa chuva também não ajudou.",
      d,
      () =>
        this.TextoFala("Vamos terminar isso logo, por onde começamos?", d, () =>
          this.TextoFala(
            "Se acalme, jovem. Não podemos ir com pressa, vamos acabar perdendo algo.",
            e,
            () =>
              this.TextoFala(
                "Vamos dar uma boa olhada nos arredores primeiramente",
                e,
                () => {
                  if (this.personagemLocal.texture.key == e) {
                    const fant = this.add.sprite(
                      this.personagemLocal.x,
                      this.personagemLocal.y - 200,
                      "fantasma",
                      0
                    );
                    fant.setAlpha(0);
                  ...
                  this.TextoFala("Viu isso?", e, () =>
                    this.TextoFala("O que?! Onde atiro?!", d, () =>
                      this.TextoFala("HaHaHaHa. Se acalme jovem.", e, () =>
                        this.TextoFala(
                          "Esse caso vai ser mais interessante do que eu esperava...",
                          e,
                          () =>
                            this.TextoFala(
                              "Ahnn.. Espero que isso seja uma coisa boa. Não esquece que fui designado para sua segurança, onde precisar que eu atire, eu atiro",
                              d,
                              () => {
                                if (this.personagemLocal.texture.key == "dan")
                                  this.fantasma.emit("comecou");
                                this.emCutscene = false;

                                this.TextoFala(
                                  "HaHaHa. Certo, agora vamos procurar pistas. A fumaça do meu cachimbo deve nos ajudar a encontrar pistas (🔎)",
                                  e
                                );
                              }
                            )
      ...
      if (this.emCutscene) {
        ...
        this.TextoFala("Terminamos?", d, () =>
          this.TextoFala("Receio que não, jovem", e, () =>
            this.TextoFala(
              "Não encontrei o suficiente para determinar o paradeiro desses desaparecidos, vamos olhar aquelas tendas",
              e,
              () => {
                this.emCutscene = false;
                this.TextoFala("OK!", d);
  ...
  EntrandoTendaE() {
    ...
    this.TextoFala(
      "Hmmm, não me parece que tenha muito pra ver por aqui",
      d,
      () =>
        this.TextoFala("Sem pressa", e, () =>
          this.TextoFala(
            "Eu trouxe minha lanterna UV, vamos dar uma olhada mais de perto",
            e,
            () =>
              this.TextoFala("Lanterna UV?", d, () => {
                this.emCutscene = false;
                this.TextoFala(
                  "Sim, com ela vou poder ver possiveis marcas ocultas a olho nu",
                  e
                );
              })
          )
  ...
  EntrandoTendaD() {
    ...
    this.TextoFala("Hmm, tiro ao alvo", e, () =>
      this.TextoFala(
        "Já que estamos aqui, que tal mostrar sua pontaria?",
        e,
        () =>
          this.TextoFala("Ahm.. Não vejo nenhum alvo", d, () =>
            this.TextoFala("Ah sim, o paranormal, eu te guio", e, () =>
              this.TextoFala("?..", d, () =>
                this.TextoFala(
                  "Presumo que tenhamos que acertar os malvados",
                  e,
                  () =>
                    this.TextoFala("Hmmm... Está bem úmido aqui..", e, () =>
                      this.TextoFala(
                        "Como sabe? Algum equipamento de investigação??",
                        d,
                        () =>
  ...
  AbrindoPortao() {
    ...
    this.TextoFala("O que será que tem la dentro?", d, () =>
      this.TextoFala("Tipo, o que tá rolando aqui exatamente?", d, () =>
        this.TextoFala(
          "HaHaHa. Está aprendenddo rápido jovem, este é o espírito de investigação",
          e,
          () =>
            this.TextoFala("...", d, () =>
              this.TextoFala(
                "Vamos la, é um ótimo primeiro caso para você",
                e,
                () =>
  ...
  update(time, delta) {
    ...
    if (
      ...
      if (this.personagemLocal.stamina > 650)
        this.personagemLocal.stamina = 650;
    ...
    this.lanternaRemota.setPosition(
      this.personagemRemoto.x,
      this.personagemRemoto.y + 15
    );
    this.lanternaRemota.setRotation(this.angleRemoto);
    this.lanternaLocal.setPosition(
      this.personagemLocal.x,
      this.personagemLocal.y + 15
    );
    this.lanternaLocal.setRotation(this.ultimoAngulo);

    let pathFinder = Phaser.Math.Angle.Between(
      this.ponteiro.x,
      this.ponteiro.y,
      this.personagemRemoto.x,
    ...
    if (
      !remotoNaTela &&
      !this.flagDentro &&
      !this.personagemRemoto.estaDentro
    ) {
      this.ponteiro.setVisible(true).setActive(true);
    } else {
      this.ponteiro.setVisible(false).setActive(false);
    }
    ...
    if (this.personagemLocal.texture.key == "dan" && this.fantasma.atacando) {
      Phaser.Math.RotateAroundDistance(
        ...
        this.personagemRemoto.y,
        this.fantasmaAngulo,
        this.fantasmaDistancia
      );
      this.fantasmaAngulo = Phaser.Math.Angle.Wrap(
        this.fantasmaAngulo + 0.00003 * norm
      );
    ...
    if (this.personagemLocal.texture.key == "ernesto") {
      ...
      if (this.fantasmaRemoto.visible && distancia < 10 && !this.invulneravel) {
      ...
      if (
        this.invulneravel &&
        agora - this.tempoUltimoDano > this.tempoInvulnerabilidade
      ) {
        this.invulneravel = false;
      }
      if (
        this.fantasmaRemoto.visible &&
        !this.fantasmaSom.isPlaying &&
        !this.flagDentro
      ) {
        this.fantasmaSom.play();
      }
    ...
    if (
      ...
      if (this.personagemLocal.cansado == false) {
        if (
          this.personagemLocal.movimento == "correndo" &&
          this.personagemLocal.stamina > 150
        ) {
          this.personagemLocal.stamina -= 1 * norm;
          if (this.personagemLocal.stamina < 150)
            this.personagemLocal.stamina = 150;
      ...
      this.areaColeta.setPosition(
        this.personagemLocal.x,
        this.personagemLocal.y
      );

      this.imagemMascaraPersonagem.setPosition(
        this.personagemLocal.x,
        this.personagemLocal.y
      );
      this.imagemMascaraLanterna.setPosition(
        this.personagemLocal.x,
        this.personagemLocal.y + 15
      );
      this.imagemMascaraLanterna.setRotation(this.ultimoAngulo);

      this.barraStamina.setPosition(
        this.personagemLocal.x - Math.cos(angle) * 30,
        this.personagemLocal.y + Math.abs(Math.cos(angle) * 30) - 70
      );
      this.barraStaminaFundo.setPosition(
        this.personagemLocal.x - Math.cos(angle) * 30,
        this.personagemLocal.y + Math.abs(Math.cos(angle) * 30) - 70
      );
      this.barraStaminaMeio.setPosition(
        this.personagemLocal.x - Math.cos(angle) * 30,
        this.personagemLocal.y + Math.abs(Math.cos(angle) * 30) - 70
      );
      ...
      if (
        pesNoChao.includes(this.frameAtual) &&
        this.personagemLocalAcao == false
      ) {
        if (!this.flagDentro) {
          this.passosSom.play();
      ...
      if (
        this.personagemLocal.texture.key == "dan" &&
        this.personagemLocal.movimento == "correndo"
      ) {
        switch (this.direcao) {
          case 0:
      ...
      if (this.personagemLocalAcao != true) {
        this.personagemLocal.setVelocity(0);
        this.personagemLocal.anims.play(
          `personagem-parado-${this.direcaoAtual}`,
          true
        );
      }
    }
        ...
        if (this.personagemLocal && this.pistas) {
          this.game.dadosJogo.send(
            JSON.stringify({
              ...
              pistas: this.pistas.map((pista) => ({
                visible: pista.objeto.visible,
              })),
    ...
    if (
      this.personagemLocal.dentroTendaE &&
      this.personagemRemoto.dentroTendaE &&
      !this.entrouTendaE
    ) {
    ...
    if (
      this.personagemLocal.dentroTendaD &&
      this.personagemRemoto.dentroTendaD &&
      !this.entrouTendaD
    ) {
